...
- Application code (e.g., the project)
...
The code is the design; verification evidence lives in the RTM. This streamlined approach requires only two documents to establish a qualified state.
...
**Example:** For the project with code in `my-app/`:
...
### 7.3 Qualification is an Event

Qualification is an **event**, not a separate document: the point at which RS and RTM are approved and all verification evidence is complete.
